...
import { journeyPhaseLabels, journeyStages, type JourneyPhase } from "@/lib/data/journey";
import { cn } from "@/lib/utils";

const phaseColor: Record<JourneyPhase, string> = {
  build: "text-signal-500 dark:text-signal-400",
  prove: "text-ember-500 dark:text-ember-400",
  launch: "text-risk-success",
};
    ...
    <section id="journey" className="relative overflow-hidden bg-canvas py-28 sm:py-36">
      <div className="container-page">
        <div className="mb-16 max-w-xl">
          <p className="font-mono text-xs uppercase tracking-[0.3em] text-accent">Chapter Five — The Journey</p>
            ...
            {journeyStages.map((stage, i) => (
              <li key={stage.title} className="relative pl-10">
                <Reveal delay={Math.min(i * 0.03, 0.24)}>
                  ...
                  <p className={cn("font-mono text-[10px] uppercase tracking-wider", phaseColor[stage.phase])}>
                    {String(i + 1).padStart(2, "0")} · {journeyPhaseLabels[stage.phase]}
                  </p>
